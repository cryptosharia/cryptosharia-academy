<script lang="ts">
	...
	const formValid = $derived(email && password && confirmPassword && passwordsMatch);
	...
	async function handleGoogleRegister() {
		...
		try {
			...
			errorMsg = userAuth.error || 'Gagal daftar dengan Google';
		} finally {
			isLoading = false;
		}
	}
	...
	async function handleEmailRegister(e: Event) {
		...
		try {
			...
			errorMsg = userAuth.error || 'Gagal mendaftar';
		} finally {
			isLoading = false;
		}
	}
...
<svelte:head>
	<title>Daftar | CryptoSharia Academy</title>
</svelte:head>
...
<div class="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-950">
	...
	<div class="flex-1 flex items-center justify-center px-4 pb-16">
		<div class="w-full max-w-md">
			<div class="mb-8">
				<h1 class="text-2xl font-extrabold text-gray-900 dark:text-white">Daftar Akun CryptoSharia</h1>
				<p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
					Sudah memiliki akun? <a href="/auth/login" class="font-semibold text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 transition-colors">Login</a>
				</p>
			</div>
			...
			<div class="space-y-3 mb-6">
				<button onclick={handleGoogleRegister} disabled={isLoading} class="w-full flex items-center justify-center gap-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-3 text-sm font-semibold text-gray-700 dark:text-gray-200 transition-all hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-gray-300 dark:hover:border-gray-600 active:scale-[0.98] disabled:opacity-50">
			...
			<!-- Form -->
			<form onsubmit={handleEmailRegister} class="space-y-5">
				<div>
				...
				<div>
					...
					<div class="relative">
						...
						<button type="button" onclick={() => (showPassword = !showPassword)} class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors" aria-label="Toggle password">
							{#if showPassword}
				...
				<div>
					<label for="confirmPassword" class="block text-sm font-semibold text-gray-900 dark:text-white mb-2">Konfirmasi Password</label>
					<div class="relative">
						<input type={showConfirmPassword ? 'text' : 'password'} id="confirmPassword" bind:value={confirmPassword} placeholder="Masukkan kembali password kamu..." required
							class="w-full rounded-xl border {confirmPassword && !passwordsMatch ? 'border-red-400 dark:border-red-500' : 'border-gray-200 dark:border-gray-700'} bg-white dark:bg-gray-800 px-4 py-3 pr-12 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 outline-none transition-all focus:ring-2 focus:ring-primary-500 focus:border-primary-500" />
						<button type="button" onclick={() => (showConfirmPassword = !showConfirmPassword)} class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors" aria-label="Toggle confirm password">
					...
					{#if confirmPassword && !passwordsMatch}
						<p class="mt-1.5 text-xs text-red-500">Password tidak cocok</p>
					{/if}
				</div>
