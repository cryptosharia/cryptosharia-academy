			...
			<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
				...
				<div class="grid gap-5 md:grid-cols-3">
					{#each content.testimonials.items as item (item.title)}
						{#if item.image}
							<div
								class="self-start overflow-hidden rounded-xl border border-slate-200 shadow-sm transition hover:border-orange-200 hover:shadow-md dark:border-slate-800 dark:hover:border-orange-900/60"
							>
								<img src={item.image} alt={item.title || item.meta} class="w-full" />
							</div>
						{:else}
							<div
								class="flex flex-col rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition hover:border-orange-200 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:hover:border-orange-900/60"
							>
								<!-- Stars -->
