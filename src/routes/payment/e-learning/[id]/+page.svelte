<script lang="ts">
	...
	const packageData = {
		...
		benefits: ['1400+ Materi Video', '1400+ Modul Praktik Portfolio', 'Sertifikat di Tiap Materi', 'Grup Komunitas'],
		terms: [
			'Video dapat dinikmati hingga jangka waktu 1 bulan setelah pembayaran terkonfirmasi oleh admin',
			'Pembelian video hanya untuk perorangan. Sertifikat disesuaikan dengan nama dan email akun CryptoSharia'
		]
	};
	...
	async function submitOrder() {
		...
		try {
			...
			const waMessage = encodeURIComponent(
				`🔔 *Pembayaran Baru CryptoSharia Academy*\n\n` +
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		...
		{:else if step === 'done'}
			<div class="max-w-lg mx-auto text-center py-16">
				...
				<p class="text-sm text-gray-500 dark:text-gray-400 mb-4">Kirimkan <strong>bukti transfer via WhatsApp</strong> ke admin. Admin akan mengkonfirmasi pembayaranmu dalam 1x24 jam.</p>
				...
				<div class="lg:col-span-2 space-y-8">
					<div class="relative rounded-3xl overflow-hidden bg-gradient-to-r from-primary-600 to-primary-700 text-white p-8 sm:p-12 shadow-lg">
						<div class="relative z-10 max-w-md">
							...
							<h1 class="text-3xl sm:text-4xl font-extrabold mb-4 leading-tight">Pelajari Ratusan Skill Bersertifikat Sekali Bayar.</h1>
							<p class="text-primary-100">Fleksibel & Praktikal. Belajar langsung dari praktisi industri Crypto Syariah.</p>
						</div>
					...
					<div class="bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-sm border border-gray-100 dark:border-gray-700">
						<span class="inline-block px-3 py-1 bg-gray-900 dark:bg-gray-700 text-white text-[10px] font-bold rounded-md mb-4 uppercase tracking-widest">Berlangganan E-learning</span>
						<h2 class="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white mb-2">{packageData.title}</h2>
						<div class="flex items-baseline gap-3 mb-8">
							...
							<span class="text-xs font-bold text-green-600 bg-green-100 dark:bg-green-900/30 px-2 py-0.5 rounded-full">Hemat 92%</span>
						</div>
						<div class="space-y-6">
							<section>
					...
					<div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-gray-700 sticky top-24">
						...
						<div class="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-xl px-4 py-3 mb-6">
							<p class="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
								<strong>⚠️ Penting:</strong> Transfer <strong>tepat</strong> sesuai nominal. Setelah transfer, klik tombol di bawah lalu <strong>kirim bukti transfer via WhatsApp</strong>.
							</p>
						</div>
						...
						<button
							...
							class="w-full bg-green-600 hover:bg-green-700 text-white rounded-xl py-4 font-bold shadow-lg transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
						>
							{#if isSubmitting}
