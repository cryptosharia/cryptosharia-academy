<script lang="ts">
	...
	interface Course { id: string; title: string; description: string; topics: number; duration: string; category: string; image: string; lessons: Lesson[]; }
...
<style>
	.course-layout { display: flex; flex-direction: row; height: calc(100vh - 4rem); }
	.video-panel { flex: 1; min-width: 0; background: #111827; }
	.video-panel iframe, .video-panel .video-placeholder { width: 100%; height: 100%; }
	.sidebar-panel { width: 340px; flex-shrink: 0; overflow-y: auto; border-left: 1px solid rgba(255,255,255,0.08); }
	@media (max-width: 768px) {
		.course-layout { flex-direction: column; height: auto; }
		.video-panel { aspect-ratio: 16/9; }
		.sidebar-panel { width: 100%; border-left: none; border-top: 1px solid rgba(255,255,255,0.08); max-height: 40vh; }
	}
</style>
...
{:else if notFound || !course}
	<div class="min-h-screen flex flex-col items-center justify-center bg-gray-950 px-4">
		...
		<a href="/subscription" class="inline-flex items-center rounded-full bg-primary-600 px-6 py-3 text-sm font-bold text-white hover:bg-primary-700 transition-all">← Kembali ke Subscription</a>
	</div>
	...
	<div class="bg-gray-950 min-h-screen pt-16">
			...
			<div class="course-layout">
				...
				<div class="video-panel">
					{#if userAuth.isLoggedIn && hasAccess}
						{#if activeVideoUrl}
							<iframe src={activeVideoUrl} title={activeLesson?.title || course.title} frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
						{:else}
					...
					{:else}
						<div class="flex flex-col items-center justify-center h-full text-center p-8 bg-gray-800/50">
							...
							<h3 class="text-white font-bold text-xl mb-2">Materi Berlangganan</h3>
							<p class="text-gray-400 text-sm mb-6 max-w-xs">Materi ini hanya dapat diakses oleh member yang memiliki langganan aktif.</p>
							<a href="/subscription#pricing" class="inline-flex items-center rounded-full bg-primary-600 px-6 py-3 text-sm font-semibold text-white hover:bg-primary-700 transition orange-glow">Mulai Berlangganan</a>
							{#if !userAuth.isLoggedIn}
								<a href="/auth/login" class="mt-3 text-xs text-gray-400 hover:text-gray-300 underline">Sudah punya akun? Login dulu</a>
							{/if}
						</div>
					{/if}
				</div>
				...
				<div class="sidebar-panel bg-gray-900">
					...
					<div class="divide-y divide-gray-800">
						{#each course.lessons as lesson, i}
							<button onclick={() => { activeLessonIndex = i; }} class="w-full flex items-center gap-3 px-4 py-3.5 text-left transition-all cursor-pointer {activeLessonIndex === i ? 'bg-primary-900/30 border-l-3 border-primary-500' : 'hover:bg-gray-800'}">
		...
		<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
			<nav class="flex items-center gap-2 text-xs text-gray-500">
				<a href="/subscription" class="hover:text-primary-400 transition-colors">Subscription</a>
				<span>›</span>
			...
			<div>
				...
				<div class="flex flex-wrap items-center gap-4 text-sm text-gray-400">
					<span class="flex items-center gap-1.5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg> {course.topics} Topik</span>
					<span class="flex items-center gap-1.5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> {course.duration}</span>
					<span class="flex items-center gap-1.5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg> {course.lessons.length} Video</span>
					<span class="px-2.5 py-0.5 rounded-md bg-gray-800 text-gray-300 text-xs font-bold">{course.category}</span>
					<span class="px-2.5 py-0.5 rounded-md bg-primary-900/50 text-primary-300 text-xs font-bold">🔒 Langganan</span>
				</div>
			</div>
			...
			{/if}
			<div>
				<a href="/subscription" class="inline-flex items-center gap-2 text-sm font-semibold text-primary-500 hover:text-primary-400 transition-colors">
					<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
					Kembali ke Katalog Subscription
				</a>
			</div>
		</div>
	</div>
