<script lang="ts">
	...
	const pricingPlans = [
		{ id: '12-bulan', name: '12 Bulan', label: 'TERPOPULER', price: 'Rp 179.000', perMonth: 'Rp 14.900/bln', features: ['1400+ Materi Video', '500+ Modul Praktik Portfolio', 'Sertifikat di Tiap Materi', 'Grup Komunitas', 'FREE Q&A Webinar Series', 'Diskon untuk Bootcamp'], popular: true },
		{ id: '6-bulan', name: '6 Bulan', label: '', price: 'Rp 99.000', perMonth: 'Rp 16.500/bln', features: ['1400+ Materi Video', '500+ Modul Praktik Portfolio', 'Sertifikat di Tiap Materi', 'Grup Komunitas', 'FREE Q&A Webinar Series', 'Diskon untuk Bootcamp'], popular: false },
		{ id: '1-bulan', name: '1 Bulan', label: '', price: 'Rp 29.000', perMonth: '', features: ['1400+ Materi Video', '500+ Modul Praktik Portfolio', 'Sertifikat di Tiap Materi', 'Grup Komunitas'], popular: false },
	];
...
<svelte:head>
	<title>Subscription | CryptoSharia Academy</title>
	<meta name="description" content="Akses semua materi e-learning tanpa batas dengan berlangganan bulanan. Lebih dari 1400 video, sertifikat, dan komunitas." />
</svelte:head>
	...
	<div class="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center sm:text-left">
		<div class="max-w-3xl">
			<div class="inline-flex items-center rounded-full border border-primary-500/30 bg-primary-500/10 px-3 py-1 text-sm font-medium text-primary-400 backdrop-blur-sm mb-6">
				<span class="flex h-2 w-2 rounded-full bg-primary-500 mr-2"></span> Langganan Bulanan — Akses Tak Terbatas
			</div>
			<h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-white mb-6 leading-tight font-sans">
				Satu Harga, <span class="text-transparent bg-clip-text bg-gradient-to-r from-primary-400 to-orange-400">Semua Materi</span>
			</h1>
			<p class="text-lg sm:text-xl text-gray-300 mb-8 max-w-2xl mx-auto sm:mx-0">
				Berlangganan dan akses 1400+ jam video pembelajaran, modul praktik, dan sertifikat — selama masa berlanggananmu aktif.
			</p>
	...
	<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
		<div class="grid grid-cols-2 gap-8 md:grid-cols-4 divide-x divide-primary-800/50">
			...
			<div class="text-center">
				...
				<div class="mt-1 text-sm font-medium text-primary-300">Modul Praktik</div>
			</div>
			<div class="text-center">
...
<section id="katalog" class="py-16 sm:py-24 bg-gray-50 dark:bg-gray-900">
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
		<div class="text-center mb-12">
			<div class="inline-flex items-center rounded-full bg-primary-100 dark:bg-primary-900/30 px-3 py-1 text-sm font-semibold text-primary-800 dark:text-primary-300 mb-4">
				🔒 Akses dengan Langganan Bulanan
			</div>
		...
		{#if isLoadingCourses}
			<div class="text-center py-16">
				...
				<p class="text-sm font-medium text-gray-500 dark:text-gray-400">Memuat materi...</p>
			</div>
		...
		{:else}
			<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
				{#each filteredCourses.slice(0, 12) as course}
					<a href="/subscription/{course.id}" class="group block">
						<div class="h-full rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col">
							<div class="h-40 bg-cover bg-center border-b border-gray-100 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 transition-transform duration-500 group-hover:scale-105 relative" style="background-image: url('{course.image || '/background-beranda.png'}');">
								<div class="absolute top-2 right-2 bg-primary-600/90 backdrop-blur-sm text-white text-[10px] font-bold px-2 py-0.5 rounded-full">🔒 Langganan</div>
							</div>
	...
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
		...
		<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
			{#each learningPaths as path}
				<a href="#pricing" class="group block">
	...
	<div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
		<div class="text-center mb-12">
			<div class="inline-flex items-center rounded-full bg-emerald-100 dark:bg-emerald-900/30 px-3 py-1 text-sm font-semibold text-emerald-800 dark:text-emerald-300 mb-4">
				✅ Akses Bulanan — Bayar Sekali, Nikmati Semua
			</div>
			<h2 class="font-sans text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white mb-3">Berlangganan Sekarang dan Jadi Lebih Hebat</h2>
			<p class="text-gray-500 dark:text-gray-400 text-sm max-w-xl mx-auto">Pilih paket yang sesuai. Bayar sekali, akses seluruh konten selama masa berlangganan aktif.</p>
		</div>
		<div class="grid sm:grid-cols-3 gap-6">
			{#each pricingPlans as plan}
				<div class="rounded-2xl {plan.popular ? 'bg-gradient-to-b from-primary-600 to-primary-700 text-white ring-2 ring-primary-500 ring-offset-2 scale-[1.03]' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700'} p-7 shadow-sm relative">
					{#if plan.label}
						<div class="absolute -top-3.5 left-1/2 -translate-x-1/2 rounded-full bg-yellow-400 px-4 py-1 text-xs font-bold text-gray-900 shadow">{plan.label}</div>
					{/if}
					<div class="text-center mb-6">
						<h3 class="font-bold text-lg mb-1">{plan.name}</h3>
						<div class="text-3xl font-extrabold">{plan.price}</div>
						{#if plan.perMonth}
							<div class="text-xs mt-1 {plan.popular ? 'text-primary-200' : 'text-gray-400'}">{plan.perMonth}</div>
						{/if}
					</div>
					<ul class="space-y-3 mb-7">
						{#each plan.features as feature}
							<li class="flex items-center gap-2.5 text-sm {plan.popular ? 'text-white/90' : 'text-gray-600 dark:text-gray-300'}">
					...
					<a href="/payment/subscription/{plan.id}" class="block text-center rounded-full {plan.popular ? 'bg-white text-primary-600 font-bold hover:bg-orange-50' : 'bg-primary-600 text-white font-semibold hover:bg-primary-700'} px-6 py-3 text-sm transition-all active:scale-95">
						Mulai Berlangganan
					</a>
				</div>
			{/each}
		...
		<!-- Compare with Units -->
		<div class="mt-10 text-center p-5 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700">
			<p class="text-sm text-gray-500 dark:text-gray-400 mb-2">Lebih suka beli materi satuan tanpa langganan?</p>
			<a href="/units" class="inline-flex items-center gap-2 text-sm font-semibold text-primary-600 dark:text-primary-400 hover:underline">
				Lihat Materi Satuan →
			</a>
		</div>
	</div>
	...
	<div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
		<div class="rounded-3xl bg-gradient-to-br from-gray-900 to-gray-800 p-8 sm:p-12 text-center relative overflow-hidden">
			...
			<div class="relative">
				...
				<h2 class="font-sans text-xl sm:text-2xl font-extrabold text-white mb-3">Subscription & Training Untuk Perusahaan</h2>
				<p class="text-gray-400 text-sm leading-relaxed max-w-xl mx-auto mb-6">Miliki akses ratusan konten serta dukungan corporate training untuk perusahaan di bidang crypto Syariah.</p>
